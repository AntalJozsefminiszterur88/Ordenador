...
class AIHandler:
    ...
    def get_calibration_coordinates(self, screen_info: dict, element_to_find: str) -> dict:
        ...
        image_data = screen_info.get("image_data", "")
        image_width = screen_info.get("width", 0)
        image_height = screen_info.get("height", 0)
                        ...
                        "content": [
                            {
                                "type": "text",
                                "text": (
                                    "Feladat: Keresd meg a '"
                                    f"{element_to_find}"
                                    "' elemet a képernyőn. "
                                    f"A kép mérete {image_width}x{image_height} pixel."
                                ),
                            },
                            {
                                "type": "image_url",
